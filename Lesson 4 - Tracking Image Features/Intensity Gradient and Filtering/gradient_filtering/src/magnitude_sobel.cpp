...
void magnitudeSobel()
{
    ...
    cv::Mat magnitude;
    cv::GaussianBlur(imgGray, magnitude, cv::Size(5, 5), 0);
    ...
    cv::filter2D(magnitude, result_x, -1, kernel_x, cv::Point(-1, -1), 0, cv::BORDER_DEFAULT);
    cv::filter2D(magnitude, result_y, -1, kernel_y, cv::Point(-1, -1), 0, cv::BORDER_DEFAULT);
